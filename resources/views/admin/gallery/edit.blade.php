...
@section('content')
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('gallery.update','test')}}" method="post" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
            ...
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photos)
                        ...
                        "{{asset('admin/pictures/gallery/' . $data->id . '/'  . $row->Filename)}}",
                    @endforeach
                ...
                initialPreviewConfig: [
                        ...
                        @foreach($data->photos as $row)
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('gallery_photo_remove_image',['data_id' => $data->id,'photo_id' => $row->id ,'photo_name' => $row->Filename, '_token' => csrf_token()])}}",
                        key: {{$row->id}}
                    },
                    @endforeach
                    @endif
    ...
    <script>
        $(function () {
            $("#image_updload_bdf").fileinput({
                ...
                initialPreview: [
                    @if($data->pdf)
                        "{{asset('admin/bdf/gallery/' . $data->id . '/'  . $data->pdf->Filename)}}"
                    @endif
                ],
                initialPreviewAsData: true,
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('gallery_remove_image',['data_id' => $data->id,'photo_id' => $data->pdf->id ,'photo_name' => $data->pdf->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->bdf->id}}
                    }
                ]
                @endif
