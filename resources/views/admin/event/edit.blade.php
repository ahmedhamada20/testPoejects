...
@section('content')
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('event.update','test')}}" method="post" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
    ...
    <script>
        $(function () {
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photo)
                        "{{asset('admin/pictures/event/' . $data->id . '/'  . $data->photo->Filename)}}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('event_photo_remove_image',['data_id' => $data->id,'photo_id' => $data->photo->id ,'photo_name' => $data->photo->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->photo->id}}
                    }
                ]
                @endif
