...
@section('content')
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('previousWork.update','test')}}" method="post" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
    ...
    <script>
        $(function () {
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photo)
                        "{{asset('admin/pictures/previousWork/' . $data->id . '/'  . $data->photo->Filename)}}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('previousWork_photo_remove_image',['data_id' => $data->id,'photo_id' => $data->photo->id ,'photo_name' => $data->photo->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->photo->id}}
                    }
                ]
                @endif
    ...
    <script>
        $(function () {
            $("#image_updload_bdf").fileinput({
                ...
                initialPreview: [
                    @if($data->pdf)
                        "{{asset('admin/bdf/previousWork/' . $data->id . '/'  . $data->pdf->Filename)}}"
                    @endif
                ],
                initialPreviewAsData: true,
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('previousWork_remove_image',['data_id' => $data->id,'photo_id' => $data->pdf->id ,'photo_name' => $data->pdf->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->pdf->id}}
                    }
                ]
                @endif
